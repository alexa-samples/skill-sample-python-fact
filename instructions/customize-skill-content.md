...
1.  **New data.** You will need to provide a set of facts for your topic.  We recommend a minimum of 25, but a total closer to 100 offers a better experience.
    ...
    2.  Search for the `FACTS` property for a specific locale, such as `en-US`. This is the data for our skill.  You can see that it is a simple list of facts.
...
2.  **New sentences to respond to your users.** There are several sentences and responses that you will want to customize for your skill.
    ...
    2.  Search for a specific locale, such as `en-US`. This is the beginning of the section where you need to customize several text strings for your skill.

    3.  **Continue through each string until you reach a new locale**  This will ensure that you cover each of the values that you need to update.
...
3.  **New language.** If you are creating this skill for another language other than English, you will need to make sure Alexa's responses are also in that language.
    ...
    *  You will also need to add the new language strings as a new locale object in **[language_strings.json](../lambda/py/language_strings.json).**
        - Copy and Paste `en-US` as a template of the new locale object to ensure you have all of the properties necessary.
